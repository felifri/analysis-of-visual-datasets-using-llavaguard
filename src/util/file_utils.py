...
def get_file_path_generator(search_dir, file_extension: str, recursive: bool = True):
    """
    Generates paths to files with the specified file extension in the specified directory.
    ...
    Yields:
    - A tuple containing:
      1. Full file path (os.PathLike object)
    ...
    if recursive:
        ...
        for root, _, files in os.walk(search_dir):
            for file in files:
                if file.lower().endswith(file_extension):  # (case-insensitive)
                    yield os.path.join(root, file), os.path.splitext(file)[0]
    else:
        # Search only the top-level directory
        for file in os.listdir(search_dir):
            full_path = os.path.join(search_dir, file)
            if os.path.isfile(full_path) and file.lower().endswith(file_extension):
                yield full_path, os.path.splitext(file)[0]