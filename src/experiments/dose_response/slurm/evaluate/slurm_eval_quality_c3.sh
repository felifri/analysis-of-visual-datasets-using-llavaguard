#!/bin/bash
#SBATCH --job-name=dose-quality-c3
...
# Compute quality metrics and ImageReward for C3 (missing from earlier runs)
# Appends C3 row to existing quality_full.csv and image_reward.csv

set -euo pipefail

echo "C3 quality evaluation started on $(hostname) at $(date)"
...
# ============================================================
# Run: Quality metrics for C3
# ============================================================

logger.info("Loading C3 images...")
c3_images = load_images_from_tar(tar_path, MAX_IMAGES)
logger.info(f"Loaded {len(c3_images)} C3 images")
...
c3_inc = inception.extract(c3_images)

result["fid_vs_c0"] = compute_fid_from_features(c3_inc, c0_inc)
result["fid_vs_train"] = compute_fid_from_features(c3_inc, train_inc)
result["fid_vs_coco"] = compute_fid_from_features(c3_inc, coco_inc)
result["kid_vs_c0"] = compute_kid_from_features(c3_inc, c0_inc)
result["kid_vs_train"] = compute_kid_from_features(c3_inc, train_inc)
result["kid_vs_coco"] = compute_kid_from_features(c3_inc, coco_inc)
logger.info(f"FID_c0={result['fid_vs_c0']:.1f}, FID_train={result['fid_vs_train']:.1f}, FID_coco={result['fid_vs_coco']:.1f}")

del inception, c0_inc, train_inc, coco_inc, c3_inc
...
c3_dino = dino.extract(c3_images)

result["fdd_vs_c0"] = compute_fid_from_features(c3_dino, c0_dino)
result["fdd_vs_train"] = compute_fid_from_features(c3_dino, train_dino)
result["fdd_vs_coco"] = compute_fid_from_features(c3_dino, coco_dino)
result["kdd_vs_c0"] = compute_kid_from_features(c3_dino, c0_dino)
result["kdd_vs_train"] = compute_kid_from_features(c3_dino, train_dino)
result["kdd_vs_coco"] = compute_kid_from_features(c3_dino, coco_dino)
result["mmd_dino_vs_c0"] = compute_mmd_rbf(c3_dino, c0_dino)
result["mmd_dino_vs_train"] = compute_mmd_rbf(c3_dino, train_dino)
result["mmd_dino_vs_coco"] = compute_mmd_rbf(c3_dino, coco_dino)
logger.info(f"FDD_c0={result['fdd_vs_c0']:.1f}, FDD_train={result['fdd_vs_train']:.1f}, FDD_coco={result['fdd_vs_coco']:.1f}")

del dino, c0_dino, train_dino, coco_dino, c3_dino, c0_images, train_images, coco_images
torch.cuda.empty_cache(); gc.collect()
...
# Remove any existing C3 row (in case of re-run)
...
logger.info(f"Appended C3 to {csv_path}")

# ============================================================
# Phase 4: ImageReward for C3
# ============================================================

logger.info("\n=== ImageReward ===")

# Reload C3 images (freed earlier)
c3_images = load_images_from_tar(tar_path, MAX_IMAGES)
...
logger.info(f"Appended C3 to {ir_csv}")
logger.info("All C3 quality metrics complete.")

PYEOF

echo "C3 quality evaluation completed at $(date)"
