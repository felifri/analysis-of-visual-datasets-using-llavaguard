...
set -euo pipefail
echo "C0 gemini training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c0_gemini" \
    "dataset@dataset.eval_dataset=train_dose_c0_gemini" \
    name=C0 group=dose-response-gemini image_size=512 \
    ...
    trainer.run_name=dose-response-C0-gemini-phase1 \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C0 gemini training complete at $(date)"
