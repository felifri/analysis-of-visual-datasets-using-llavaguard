...
echo "C3 full training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c3" \
    "dataset@dataset.eval_dataset=train_dose_c3" \
    name=C3 group=dose-response-full image_size=512 \
    global_batch_size=256 device_train_microbatch_size=32 \
    trainer.max_duration=100_000ba \
    trainer.save_folder=${CKPT_DIR}/C3/phase1 \
    trainer.run_name=dose-response-C3-full \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C3 full training complete at $(date)"
