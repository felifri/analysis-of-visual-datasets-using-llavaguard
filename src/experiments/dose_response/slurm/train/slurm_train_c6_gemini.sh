...
set -euo pipefail
echo "C6 gemini training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c6_gemini" \
    "dataset@dataset.eval_dataset=train_dose_c6_gemini" \
    name=C6 group=dose-response-gemini image_size=512 \
    ...
    trainer.run_name=dose-response-C6-gemini-phase1 \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C6 gemini training complete at $(date)"
