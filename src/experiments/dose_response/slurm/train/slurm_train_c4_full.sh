...
echo "C4 full training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    name=C4 group=dose-response-full image_size=512 \
    ...
    trainer.run_name=dose-response-C4-full-phase1 \
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c4" \
    "dataset@dataset.eval_dataset=train_dose_c4" \
    name=C4 group=dose-response-full image_size=1024 \
    global_batch_size=128 device_train_microbatch_size=16 \
    trainer.max_duration=20_000ba \
    trainer.save_folder=${CKPT_DIR}/C4/phase2 \
    trainer.run_name=dose-response-C4-full-phase2 \
    +trainer.load_weights_only=true +trainer.load_path=${CKPT_DIR}/C4/phase1/latest-rank0.pt \
    "~algorithms.repa" \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C4 full training complete at $(date)"
