...
set -euo pipefail
echo "C2 gemini training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c2_gemini" \
    "dataset@dataset.eval_dataset=train_dose_c2_gemini" \
    name=C2 group=dose-response-gemini image_size=512 \
    ...
    trainer.run_name=dose-response-C2-gemini-phase1 \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C2 gemini training complete at $(date)"
