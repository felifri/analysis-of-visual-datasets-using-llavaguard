...
set -euo pipefail
echo "C5 training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c5" \
    "dataset@dataset.eval_dataset=train_dose_c5" \
    name=C5 group=dose-response image_size=512 \
    global_batch_size=256 device_train_microbatch_size=32 \
    trainer.max_duration=100_000ba \
    trainer.save_folder=${CKPT_DIR}/C5/phase1 \
    trainer.run_name=dose-response-C5-phase1 \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C5 training complete at $(date)"
