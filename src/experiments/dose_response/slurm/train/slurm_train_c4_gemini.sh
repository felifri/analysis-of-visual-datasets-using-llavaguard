...
set -euo pipefail
echo "C4 gemini training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    "dataset@dataset.train_dataset=train_dose_c4_gemini" \
    "dataset@dataset.eval_dataset=train_dose_c4_gemini" \
    name=C4 group=dose-response-gemini image_size=512 \
    ...
    trainer.run_name=dose-response-C4-gemini-phase1 \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C4 gemini training complete at $(date)"
