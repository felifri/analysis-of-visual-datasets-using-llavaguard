...
set -euo pipefail
echo "C4 Phase 2 started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    +trainer.load_weights_only=true +trainer.load_strict_model_weights=false +trainer.load_path=${CKPT_DIR}/C4/phase1/latest-rank0.pt \
    "~algorithms.repa" \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C4 Phase 2 complete at $(date)"
