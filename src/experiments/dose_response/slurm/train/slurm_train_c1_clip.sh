...
set -euo pipefail
echo "C1-CLIP training started on $(hostname) at $(date)"
...
$COMPOSER -n 8 -m prx.training.train \
    ...
    name=C1_clip group=dose-response-clip image_size=512 \
    global_batch_size=256 device_train_microbatch_size=32 \
    trainer.max_duration=100_000ba \
    trainer.save_folder=${CKPT_DIR}/C1_clip/phase1 \
    trainer.run_name=dose-response-C1-clip-phase1 \
    "~algorithms.repa" "~algorithms.pdino" \
    eval_first=false trainer.eval_interval=0 seed=42

echo "C1-CLIP Phase 1 complete at $(date)"
