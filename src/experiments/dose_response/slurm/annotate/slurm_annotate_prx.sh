#!/bin/bash
#SBATCH --job-name=dose-annot-prx
...
# Annotate 10K generated images per PRX model with LlavaGuard
# Each task handles one model, runs SGLang server with dp_size=8

set -euo pipefail

MODELS=(prx-1024-beta prx-512-base prx-512-sft prx-512-sft-distilled prx-512-dc-ae prx-256-base prx-256-sft)
MODEL=${MODELS[$SLURM_ARRAY_TASK_ID]}

echo "Job $SLURM_JOB_ID task $SLURM_ARRAY_TASK_ID ($MODEL) started on $(hostname) at $(date)"
...
# Copy model to local /tmp on first use
...
logger = logging.getLogger('annotate_prx')
...
logger.info(f'${MODEL}: {len(todo_paths)} to annotate ({len(existing)} exist, {len(paths)} total)')
...
for i in range(0, len(todo_paths), batch_size):
    ...
    logger.info(f'${MODEL}: {i+len(batch_p)}/{len(todo_paths)} ({total_invalid} invalid)')

# Compress to parquet
compress_annotations(output_dir, os.path.dirname(output_dir), parquet_size=100000)
logger.info(f'${MODEL}: annotation complete')
"
...
echo "$MODEL annotation completed at $(date)"
