#!/bin/bash
#SBATCH --job-name=dose-annot-tsms
...
# Annotate multi-gen-seed images from training-seed checkpoints SEQUENTIALLY.
# 8 training-seed conditions × 4 generation seeds = 32 runs.
# Gen seed=42 already annotated from previous job.
...
# Process each (training_condition, gen_seed) sequentially
for TRAIN_COND in C1_seed_137 C1_seed_314 C1_seed_789 C1_seed_1331 C0_seed_137 C0_seed_314 C0_seed_789 C0_seed_1331; do
    for GEN_SEED in 137 314 789 1331; do
        MODEL_ID="dose_${TRAIN_COND}_genseed_${GEN_SEED}"
        GEN_DIR="<your folder>"
        ANNOT_DIR="<your folder>"

        # Skip if parquet already exists
        if ls "${ANNOT_DIR}"/*.parquet 1>/dev/null 2>&1 || [ -f "<your folder>" ]; then
        ...
        if [ ! -f "${GEN_DIR}/images.tar" ] && [ ! -d "${GEN_DIR}/images" ]; then
            echo "=== Skipping ${MODEL_ID}: no images found ==="
            continue
        fi
...
logger = logging.getLogger('annotate_tsms')
...
"
        ...
        # Remove unpacked images to free space
        echo "Cleaning up ${GEN_DIR}/images..."
        rm -rf "${GEN_DIR}/images"
        # Remove individual JSONs now that parquet exists
        rm -f "${ANNOT_DIR}"/*.json "${ANNOT_DIR}"/*.txt 2>/dev/null
        rmdir "${ANNOT_DIR}" 2>/dev/null || true
...
echo "All training-seed × gen-seed annotations completed at $(date)"
