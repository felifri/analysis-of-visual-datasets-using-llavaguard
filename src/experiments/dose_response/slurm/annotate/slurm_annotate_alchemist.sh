#!/bin/bash
# Annotate the Alchemist SFT dataset with LlavaGuard to measure its safety composition
#
#SBATCH --job-name=annot-alch
...
echo "Annotating Alchemist dataset on $(hostname) at $(date)"
...
# Copy LlavaGuard model to local /tmp
...
logger = logging.getLogger('annotate_alchemist')
...
logger.info(f'Alchemist: {len(todo_paths)} to annotate ({len(existing)} exist, {len(paths)} total)')
...
for i in range(0, len(todo_paths), batch_size):
    ...
    logger.info(f'Alchemist: {i+len(batch_p)}/{len(todo_paths)} ({total_invalid} invalid)')
...
# Print summary
import pandas as pd
pq = os.path.join(os.path.dirname(output_dir), 'alchemist_dataset.parquet')
if os.path.exists(pq):
    df = pd.read_parquet(pq)
    total = len(df)
    unsafe = (df['rating'] == 'Unsafe').sum()
    logger.info(f'Alchemist dataset safety: {unsafe}/{total} unsafe ({unsafe/total*100:.2f}%)')
    cats = df[df['rating'] == 'Unsafe']['category'].value_counts()
    for cat, cnt in cats.items():
        logger.info(f'  {cat}: {cnt} ({cnt/unsafe*100:.1f}%)')

logger.info('Alchemist annotation complete')
"
...
echo "Alchemist annotation completed at $(date)"
