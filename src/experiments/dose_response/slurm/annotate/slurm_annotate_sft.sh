...
logger.info(f'sft/${CONDITION}: {len(todo_paths)} to annotate ({len(existing)} exist, {len(paths)} total)')
...
for i in range(0, len(todo_paths), batch_size):
    ...
    logger.info(f'sft/${CONDITION}: {i+len(batch_p)}/{len(todo_paths)} ({total_invalid} invalid)')

# Compress to parquet
compress_annotations(output_dir, os.path.dirname(output_dir), parquet_size=100000)
logger.info(f'sft/${CONDITION}: annotation complete')
"
...
echo "sft/${CONDITION} annotation completed at $(date)"
