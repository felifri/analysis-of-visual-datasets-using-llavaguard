...
# Annotate 10K generated images from dose-response checkpoints with LlavaGuard
# Array mapping:
#   0: C1 (0% unsafe)
#   1: C2 (5% unsafe)
#   2: C0 (original ~1.21% unsafe)
#   3: C4 (1.21% at 1M scale)
#   4: C6 (9.6% at 1M scale)
...
if [ ! -d "${GEN_DIR}/images" ]; then
    ...
    echo "Run slurm_generate_dose_response.sh first."
    exit 1
fi
...
logger = logging.getLogger('annotate_dose_response')
...
logger.info(f'${CONDITION}: {len(todo_paths)} to annotate ({len(existing)} exist, {len(paths)} total)')
...
for i in range(0, len(todo_paths), batch_size):
    ...
    logger.info(f'${CONDITION}: {i+len(batch_p)}/{len(todo_paths)} ({total_invalid} invalid)')

# Compress to parquet
compress_annotations(output_dir, os.path.dirname(output_dir), parquet_size=100000)
logger.info(f'${CONDITION}: annotation complete')
"
...
echo "$CONDITION annotation completed at $(date)"
