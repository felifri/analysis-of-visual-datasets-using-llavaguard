# LlavaGuard Inference on the CC12M Dataset

This experiment downloads the CC12M dataset from Huggingface. It then evaluates its safety using LlavaGuard.

## Download Dataset
* Run `conda deactivate && conda activate /pfss/mlde/workspaces/mlde_wsp_KIServiceCenter/finngu/envs/dataset_download`
* Run `python src/experiments/datasets/cc12m/entrypoint_download.py download` to download via the huggingface `datasets` library
* Run `python src/experiments/datasets/cc12m/entrypoint_download.py validate` to check if all images can be accessed without error
...
* Use 1 server running LlavaGuard-7B on 4 GPUs each (after 4 GPUs the sglang servers become less efficient)
    ...
    * Run `CUDA_VISIBLE_DEVICES=0,1,2,3 python src/experiments/datasets/cc12m/entrypoint_inference_server.py`
...
* Use 1 client to send requests to the servers
    ...
    * Run `conda deactivate && conda activate /pfss/mlde/workspaces/mlde_wsp_KIServiceCenter/finngu/envs/sglang`
    * Run `python src/experiments/datasets/cc12m/entrypoint_inference_client.py`
    * Detach from the `screen` session (`Ctrl+A`, then `D`)
...
* Run `conda deactivate && conda activate /pfss/mlde/workspaces/mlde_wsp_KIServiceCenter/finngu/envs/dataset_download`
* Run `python src/experiments/datasets/cc12m/entrypoint_compress_annotations.py`
